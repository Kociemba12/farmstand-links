...
export function detectCategoryFromQuery(query: string): string | null {
  ...
  // Check if query matches any category label directly
 ...
 * Determines how the Map should handle the search results
 */
export function classifySearchQuery(
  ...
  // 1) Check for exact/strong farmstand name match first
  ...
  // Check for starts-with name match
  ...
  // 2) Check for product/category search
  ...
  // 3) Check for location search (zip code)
  if (/^\d{5}$/.test(trimmed)) {
    // Look for farmstands with this zip code to get coordinates
  ...
  // 4) Check for city/location match
