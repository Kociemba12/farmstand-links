...
// Used when no location has been saved for the current user.
// Empty city/state prevents inheriting another user's location.
const BLANK_LOCATION: LocationSettings = {
  city: '',
  state: '',
  latitude: 0,
  longitude: 0,
...
export const useUserStore = create<UserState>((set, get) => ({
  ...
  loadUser: async () => {
    ...
    try {
      ...
      if (isLoggedIn === 'true' && userData) {
        ...
        // === SESSION GATE: When Supabase is configured, ALL non-guest users must have
        // a valid Supabase session. This covers:
        //   - Real UUID users (standard Supabase auth)
        //   - null-id users (admin logged in via local fallback — THIS was the bug)
        //   - user- prefix users (local-only registrations)
        // Only guests (id === 'guest') are exempt since they intentionally have no session.
        const isGuestUser = user.id === 'guest';
        if (!isGuestUser && isSupabaseConfigured()) {
          const supabaseSession = await getValidSession();
          if (!supabaseSession) {
            // Session gone — clear the stale flag so user sees login screen
            console.log('[UserStore] loadUser: Supabase session missing (user.id=' + String(user.id) + ') — clearing stale farmstand_logged_in flag');
        ...
        set({
          ...
          location: locData ? JSON.parse(locData) : BLANK_LOCATION,
  ...
  signOut: async () => {
    ...
    await AsyncStorage.removeItem('farmstand_logged_in');
    await AsyncStorage.removeItem('farmstand_location'); // Never leak one user's location to the next
