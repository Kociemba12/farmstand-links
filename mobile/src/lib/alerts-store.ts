...
export const useAlertsStore = create<AlertsStore>((set, get) => ({
  ...
  loadAlerts: async (_userId?: string) => {
    // inFlight guard: skip if a fetch is already running (prevents flicker from
    // overlapping focus/AppState triggers especially in offline / airplane mode)
    if (get().isLoading) {
      console.log('[Alerts] loadAlerts skipped — already in flight');
      return;
    }
