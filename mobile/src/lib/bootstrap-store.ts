...
async function fetchUserFarmstandsFromSupabase(userId: string, accessToken?: string): Promise<{
  ...
  try {
    ...
    // Use the provided token if available and not expired, otherwise try getValidSession().
    // CRITICAL: AuthProvider may pass an expired token when its own refresh failed on cold
    // launch (keeps expired session so the user stays logged in). Sending an expired JWT
    // to Supabase causes auth.uid() to return null → RLS blocks → 0 rows returned → Profile
    // shows "Add a Farmstand" even though the user has farmstands. Always validate the
    // token's expiry here regardless of source.
    let resolvedToken = accessToken;
    if (resolvedToken) {
      // Decode expiry from the JWT payload (second segment, base64-encoded JSON)
      try {
        const payloadB64 = resolvedToken.split('.')[1];
        if (payloadB64) {
          const payload = JSON.parse(atob(payloadB64)) as { exp?: number };
          const nowSeconds = Math.floor(Date.now() / 1000);
          if (payload.exp && nowSeconds >= payload.exp) {
            console.log('[Bootstrap] Provided access_token is expired (exp:', payload.exp, ', now:', nowSeconds, ') — attempting session refresh before query');
            resolvedToken = undefined; // Fall through to getValidSession() which refreshes
          }
        }
      } catch {
        // If we can't decode the token, let getValidSession() sort it out
        console.log('[Bootstrap] Could not decode provided token — deferring to getValidSession()');
...
function mapSupabaseFarmstand(row: Record<string, unknown>): Farmstand {
  ...
  return {
    ...
    operatingStatus: get('operating_status', 'operatingStatus', 'open') as import('./farmer-store').OperatingStatus,
    ...
    premiumStatus: get('premium_status', 'premiumStatus', 'free') as 'free' | 'trial' | 'active' | 'expired',
...
export const useBootstrapStore = create<BootstrapState>((set, get) => ({
  ...
  bootstrap: async () => {
    ...
    try {
      ...
      // NOTE: loadFavorites() is intentionally included here so that favorites are
      // fully hydrated BEFORE appReady = true (which dismisses the splash screen).
      // This prevents any flash of unselected hearts or empty Saved tab.
      // user is guaranteed to be set at this point (Step 2 completed).
      ...
      const [fetchResult] = await Promise.all([
        ...
        // Load alerts so the Inbox tab badge shows immediately on startup
        useAlertsStore.getState().loadAlerts(user.id ?? undefined),

        // Load favorites — must complete before splash dismisses so hearts and
        // Saved tab are correct on first visible render.
  ...
  refreshUserFarmstands: async (accessToken?: string) => {
    ...
    try {
      ...
      // Apply visibility overrides from backend storage.
      // The `show_on_map` column doesn't exist in Supabase yet, so showOnMap always
      // defaults to true on Supabase load. The backend JSON store holds explicit overrides.
      const backendUrl = process.env.EXPO_PUBLIC_VIBECODE_BACKEND_URL ?? '';
      if (backendUrl && freshFarmstands.length > 0) {
        try {
          const visResp = await fetch(`${backendUrl}/api/farmstand-visibility`);
          const visCt = visResp.headers.get('content-type') ?? '';
          if (visResp.ok && visCt.includes('application/json')) {
            const visData = await visResp.json() as { success: boolean; visibility: Record<string, boolean> };
            const visMap = visData.visibility ?? {};
            for (const fs of freshFarmstands) {
              if (fs.id in visMap) {
                const overrideVal = visMap[fs.id];
                console.log(`[Bootstrap][Visibility] Override ${fs.id} (${fs.name}): showOnMap=${overrideVal} (Supabase default was ${fs.showOnMap})`);
                fs.showOnMap = overrideVal as boolean;
              }
            }
            console.log('[Bootstrap][Visibility] Applied', Object.keys(visMap).length, 'visibility override(s)');
          }
        } catch (visErr) {
          console.log('[Bootstrap][Visibility] Failed to fetch visibility overrides:', visErr);
        }
      }
      ...
      // Sync into admin store: upsert the user's fresh farmstands by ID.
      // IMPORTANT: do NOT filter out records by ownerUserId/claimedByUserId —
      // loadAdminData() may still be in flight with the full farmstand list.
      // Replacing the list based on a partial allFarmstands snapshot would
      // discard all other farms and leave Explore with only the user's own
      // farmstand(s) until the next loadAdminData() completes.
      // Instead: merge by ID so we only update what we just fetched.
      const adminStore = useAdminStore.getState();
      const farmstandMap = new Map(adminStore.allFarmstands.map((f) => [f.id, f]));
      for (const farmstand of freshFarmstands) {
        farmstandMap.set(farmstand.id, farmstand);
      }
      useAdminStore.setState({ allFarmstands: Array.from(farmstandMap.values()) });
    } catch (refreshErr) {
