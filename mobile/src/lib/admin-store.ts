...
import { Farmstand, FarmstandStatus, GoldVerifiedSource, OwnershipDisputeStatus } from './farmer-store';
...
import { supabase, isSupabaseConfigured, hasActiveSession, refreshSupabaseSession, ensureSessionReady, getValidSession, getSupabaseUrl } from './supabase';
...
export const useAdminStore = create<AdminState>((set, get) => ({
  ...
  loadAdminData: async () => {
    ...
    if (cached.data.length > 0) {
      // Only update allFarmstands if we don't already have live data in memory
      const existing = get().allFarmstands;
    ...
    try {
      ...
      // Load stored users or initialize with mock data
      let users: AdminUser[];
      if (storedUsers) {
  ...
  refreshSingleFarmstand: async (id: string): Promise<Farmstand | null> => {
    ...
    try {
      ...
      console.log('[AdminStore] FETCH SUCCESS refreshSingleFarmstand:', refreshedFarmstand.name, '| id:', id);
  ...
  updateFarmstand: async (id: string, updates: Partial<Farmstand>) => {
    ...
    if (isSupabaseConfigured()) {
      ...
      //   directionsNotes, parkingNotes, todaysNote, operationalStatus, showOnMap, isActive,
      ...
      if (supabaseError && (supabaseError as any).code === '42501') {
        ...
        const backendResp = await fetch(`${backendUrl}/api/update-farmstand`, {
          method: 'POST',
          headers: {
        ...
        if (!backendResp.ok || !backendData.success) {
          console.error('[AdminStore] Backend update failed:', backendResp.status, backendData.error);
          throw new Error(backendData.error || 'Failed to update farmstand');
        }
        ...
        const session2 = await getValidSession();
        if (!session2?.access_token) {
          throw new Error('Session expired. Please sign in again.');
        }
        const backendResp2 = await fetch(`${backendUrl2}/api/update-farmstand`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
        ...
        if (!backendResp2.ok || !backendData2.success) {
          console.error('[AdminStore] Backend update failed (error fallback):', backendResp2.status, backendData2.error);
          throw new Error(backendData2.error || 'Failed to update farmstand');
        }
        ...
        const session3 = await getValidSession();
        if (!session3?.access_token) {
          throw new Error('Session expired. Please sign in again.');
        }
        const backendResp3 = await fetch(`${backendUrl3}/api/update-farmstand`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${session3.access_token}`,
          },
          body: JSON.stringify({ farmstand_id: id, updates: snakeCaseUpdates }),
        });
        const backendData3 = await backendResp3.json() as { success: boolean; error?: string };
        if (!backendResp3.ok || !backendData3.success) {
          console.error('[AdminStore] Backend update failed (0-rows fallback):', backendResp3.status, backendData3.error);
          throw new Error(backendData3.error || 'Failed to update farmstand');
        }
  ...
  deleteFarmstand: async (id: string) => {
    ...
    if (!data || data.length === 0) throw new Error('Soft delete failed: no rows updated (bad id or RLS blocked)');

    // Remove from local state
    const farmstands = get().allFarmstands.filter(f => f.id !== id);
    set({ allFarmstands: farmstands });

    // Refresh admin UI counts/list
  ...
  createFarmstand: async (farmstand: Omit<Farmstand, 'id' | 'updatedAt'> & { id?: string }) => {
    ...
    if (isSupabaseConfigured()) {
      ...
      console.log('[AdminStore] Supabase insert success, new ID:', newId);
