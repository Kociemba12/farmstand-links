...
import { setFetchInFlight, CACHE_KEYS } from '@/lib/farmstand-cache';
    ...
    <Animated.View entering={FadeInDown.delay(delay).duration(400)}>
      ...
      >
        <View className="flex-row items-start justify-between">
          <View className="flex-1 mr-3">
            ...
            <View className="flex-row items-center justify-between pt-2 border-t border-stone-100">
              ...
              >
                <Text style={{ fontSize: 12, fontWeight: '600', color: '#3D2E22' }}>Actions</Text>
                <ChevronDown size={11} color="#3D2E22" />
              </Pressable>
            </View>
          </View>
        </View>
          ...
          >
            <View className="bg-white rounded-t-[28px] pt-3 pb-8">
              ...
              >
                ...
                <Text className="text-base text-stone-700 flex-1">Review Farmstand</Text>
              ...
              <Pressable
                onPress={() => {
                  setShowMenu(false);
                  console.log(`[AdminContactInfo] opening for farmstandId: ${farmstand.id}`);
                  console.log(`[AdminContactInfo] selected farmstand phone: ${farmstand.internalContactPhone ?? 'null'}`);
                  console.log(`[AdminContactInfo] selected farmstand email: ${farmstand.internalContactEmail ?? 'null'}`);
                  console.log(`[AdminContactInfo] source query includes internal fields: yes`);
                  setShowContact(true);
                }}
                className="flex-row items-center px-5 py-4 active:bg-stone-50"
              >
                ...
                <Text className="text-base text-stone-700 flex-1">Contact</Text>
                <ChevronRight size={18} color="#A8A29E" />
              </Pressable>
        ...
        >
          ...
          >
            <View className="bg-white rounded-t-[28px] pt-3 pb-10">
              <View className="w-12 h-1.5 bg-stone-300 rounded-full self-center mb-5" />
              <Text className="text-lg font-bold text-stone-900 px-5 mb-0.5">Contact Info</Text>
              <Text className="text-xs text-stone-400 px-5 mb-5">
                Private admin-only outreach information. Not shown publicly.
              </Text>

              {/* Phone row */}
              <View className="mx-5 mb-3 p-4 bg-stone-50 rounded-2xl">
                <View className="flex-row items-center mb-1">
                  <Phone size={14} color="#0D9488" style={{ marginRight: 6 }} />
                  <Text className="text-xs font-semibold text-stone-500 uppercase tracking-wide">Phone</Text>
                </View>
                {farmstand.internalContactPhone ? (
                  <View className="flex-row items-center justify-between mt-1">
                    <Text className="text-base text-stone-800 font-medium flex-1">
                      {farmstand.internalContactPhone}
                    </Text>
                    <Pressable
                      onPress={() => {
                        Linking.openURL(`tel:${farmstand.internalContactPhone}`).catch(() => {
                          console.log('[AdminContactInfo] failed to open phone dialer');
                        });
                      }}
                      className="ml-3 px-4 py-2 bg-teal-600 rounded-xl active:bg-teal-700"
                    >
                      <Text className="text-white text-sm font-semibold">Call</Text>
                    </Pressable>
                  </View>
                ) : (
                  <Text className="text-base text-stone-400 mt-1">Not provided</Text>
                )}
              </View>

              {/* Email row */}
              <View className="mx-5 mb-5 p-4 bg-stone-50 rounded-2xl">
                <View className="flex-row items-center mb-1">
                  <Mail size={14} color="#6366F1" style={{ marginRight: 6 }} />
                  <Text className="text-xs font-semibold text-stone-500 uppercase tracking-wide">Email</Text>
                </View>
                {farmstand.internalContactEmail ? (
                  <View className="flex-row items-center justify-between mt-1">
                    <Text className="text-base text-stone-800 font-medium flex-1" numberOfLines={1}>
                      {farmstand.internalContactEmail}
                    </Text>
                    <Pressable
                      onPress={() => {
                        Linking.openURL(`mailto:${farmstand.internalContactEmail}`).catch(() => {
                          console.log('[AdminContactInfo] failed to open email client');
                        });
                      }}
                      className="ml-3 px-4 py-2 bg-indigo-500 rounded-xl active:bg-indigo-600"
                    >
              ...
              <Pressable
                onPress={() => setShowContact(false)}
                className="mx-5 py-3.5 bg-stone-100 rounded-2xl items-center"
              >
                <Text className="text-base font-medium text-stone-600">Done</Text>
              </Pressable>
            </View>
          </Pressable>
        </Modal>
...
function ManageFarmstandsContent() {
  ...
  const handleDelete = async () => {
    ...
    try {
      // Soft delete via admin store (sets deleted_at, falls back to backend if RLS blocks)
      await deleteFarmstand(idToDelete);
  ...
  const handleViewOnMap = (farmstand: Farmstand) => {
    console.log('[ViewOnMap] Selected farmstand id:', farmstand.id, '| coords:', farmstand.latitude, farmstand.longitude, '| name:', farmstand.name);
    if (farmstand.latitude && farmstand.longitude) {
      router.push({
        pathname: '/(tabs)/map',
        params: {
          viewOnMapFarmstandId: farmstand.id,
          viewOnMapLat: farmstand.latitude.toString(),
          viewOnMapLng: farmstand.longitude.toString(),
          viewOnMapName: farmstand.name,
        },
      });
    } else if (farmstand.id) {
      // No coords but have ID - navigate and let map try to find it
      console.log('[ViewOnMap] No coords, navigating by ID only:', farmstand.id);
      router.push({
        pathname: '/(tabs)/map',
        params: {
          viewOnMapFarmstandId: farmstand.id,
  ...
  const handleApprove = async (farmstand: Farmstand) => {
    ...
    try {
      ...
      console.log('[handleApprove] safeApproveFarmstand succeeded for:', farmstand.id);

      // ── Optimistic update: immediately flip this farmstand to active in the store ──
      // This ensures the card reflects the new state before the re-fetch completes.
      const currentFarmstands = useAdminStore.getState().allFarmstands;
      const updatedFarmstands = currentFarmstands.map((f) =>
        f.id === farmstand.id ? { ...f, status: 'active' as FarmstandStatus } : f
      );
      useAdminStore.setState({ allFarmstands: updatedFarmstands });
      console.log('[handleApprove] Optimistic update applied — status set to active for:', farmstand.id);
      console.log('[handleApprove] Pending count after optimistic update:',
        updatedFarmstands.filter((f) => f.status === 'pending').length,
        '| Active count:', updatedFarmstands.filter((f) => f.status === 'active').length);

      await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      showToast('Listing approved!', 'success');

      // ── Force a clean re-fetch so UI reflects true server state ──
      // Reset farmstandsSource so loadAdminData's merge logic treats the
      // fresh network data as authoritative (not overwritten by stale in-store record).
      useAdminStore.setState({ farmstandsSource: 'empty' });
      // Clear any in-flight guard that might block the re-fetch
      setFetchInFlight(CACHE_KEYS.EXPLORE, false);
      await loadAdminData();
      console.log('[handleApprove] Re-fetch complete — pending count:',
        useAdminStore.getState().allFarmstands.filter((f) => f.status === 'pending').length,
        '| Active count:', useAdminStore.getState().allFarmstands.filter((f) => f.status === 'active').length);
    } catch (error) {
      await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
