...
function AdminDashboardContent() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      loadAnalytics();
      loadAdminData(); // Still load full data for other counts
      loadClaimRequestCount(); // Fetch claim count from Supabase
      loadNewFeedbackCount(); // Fetch unread feedback count from backend
    }, [loadAnalytics, loadAdminData, loadClaimRequestCount, loadNewFeedbackCount])
  );

  // Pull-to-refresh handler
  const handleRefresh = useCallback(async () => {
    setRefreshing(true);
    await Promise.all([loadAnalytics(), loadAdminData(), loadClaimRequestCount(), loadNewFeedbackCount()]);
    setRefreshing(false);
  }, [loadAnalytics, loadAdminData, loadClaimRequestCount, loadNewFeedbackCount]);

  // Use analytics for main counts (prevents flicker)
  const approxLocationCount = allFarmstands.filter(
    (f) => f.locationPrecision?.startsWith('approximate') && f.adminReviewReason === 'approx_location'
  ).length;
      ...
      >
        ...
        <View className="px-5 pt-6">
          ...
          <DashboardCard
            ...
            description="Verify farmstands with approximate pins"
            icon={<MapPinned size={24} color="#0891b2" />}
            count={approxLocationCount}
            onPress={() => router.push('/admin/approx-locations')}
            iconBgColor="#CFFAFE"
            delay={325}
          />
