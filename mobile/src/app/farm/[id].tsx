...
import {
  ...
  Navigation, ChevronRight, Leaf, X, Camera, ShieldCheck, UserCheck, Package, ChevronDown, MessageSquare, AlertTriangle, Clock as ClockIcon
...
export default function FarmDetailScreen() {
  const { id, openClaimModal: openClaimModalParam } = useLocalSearchParams<{ id: string; openClaimModal?: string }>();
  ...
  useFocusEffect(
    ...
    }, [id, refreshSingleFarmstand, loadProducts, fetchFreshClaimState])
  );
  ...
  }, [id, fetchFreshClaimState, refreshSingleFarmstand]);
  ...
  // Open claim modal if requested via param (from onboarding flow)
  useEffect(() => {
    if (openClaimModalParam === 'true' && farmstand && !showClaimModal) {
  ...
  const farm = useMemo(() => {
    ...
    // Get hero image using unified image helper
  ...
  const handleShare = async () => {
    ...
    // Build deep link URL for the farmstand using farmstand.online domain
    const deepLinkUrl = `https://farmstand.online/farm/${id}`;

    try {
      await Share.share(
        {
          // iOS uses 'url' for the preview card - domain shows as "Farmstand.online"
          url: deepLinkUrl,
          // 'title' appears as the preview title
          title: farm.name,
          // 'message' is the share content / subtitle
          message: `Check out ${farm.name} on Farmstand`,
        },
        {
  ...
  const handleSubmitReview = async () => {
    ...
    // Save to reviews store (existing behavior)
    await addReview(id, user.name, reviewRating, reviewText.trim());
  ...
  const handleClaimSuccess = async () => {
    ...
    // Immediately re-fetch claim state from Supabase to update UI
    await fetchFreshClaimState();
    // Also refresh farmstand data
    await loadAdminData();
    Alert.alert(
      'Farmstand Claimed!',
      'You are now the owner of this farmstand. You can manage its listings, hours, and details.',
      [{ text: 'OK' }]
    );
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
        ...
        <Pressable onPress={() => !farm.isAIGeneratedImage && handleOpenPhotoGallery(0)}>
          <View style={styles.heroContainer}>
            ...
            {/* Photo count badge - Bottom Right - Only show for non-AI images */}
            {!farm.isAIGeneratedImage && farm.photos && farm.photos.length > 1 && (
              <View style={styles.photoCountBadge}>
                <Camera size={14} color="#FDF8F3" />
                <Text style={styles.photoCountText}>{farm.photos.length}</Text>
              </View>
            )}
              ...
              <View style={styles.headerRight}>
                ...
                <Pressable onPress={handleFavoritePress} style={styles.headerButton}>
                  <Animated.View style={heartAnimatedStyle}>
                    <Heart
                      size={20}
                      color={isFavorite ? '#C45C3E' : '#3D3D3D'}
        ...
        <View style={styles.content}>
          ...
          <Animated.View entering={FadeInDown.delay(100).duration(400)} style={styles.titleSection}>
            ...
            <View style={styles.ratingRow}>
              ...
              <Text style={styles.reviewCount}>({farm.reviewCount + reviews.length} reviews)</Text>
            </View>
          </Animated.View>
          ...
          {/* Claim Pending Banner - Compact banner near top when claim is pending */}
          {hasPendingClaimRequest && (
            <Animated.View entering={FadeInDown.delay(160).duration(300)} style={styles.pendingBanner}>
              <View style={styles.pendingBannerContent}>
                <ClockIcon size={14} color="#B45309" />
                <View style={styles.pendingBannerTextContainer}>
                  <Text style={styles.pendingBannerTitle}>Claim pending</Text>
                  <Text style={styles.pendingBannerSubtext}>We're reviewing your request</Text>
                </View>
              </View>
              <View style={styles.pendingBannerPill}>
                <Text style={styles.pendingBannerPillText}>Pending</Text>
              </View>
            </Animated.View>
          )}

          {/* TEMP DEBUG PANEL - Claim state diagnostics for TestFlight */}
          {__DEV__ && user?.id ? (
            <View style={{ backgroundColor: '#1a1a2e', padding: 10, borderRadius: 8, marginBottom: 12, marginTop: 4 }}>
              ...
              <Text style={{ color: '#aaffcc', fontFamily: 'monospace', fontSize: 9, lineHeight: 14 }}>
                {`freshClaimState.isLoading: ${freshClaimState.isLoading}`}
              </Text>
              <Text style={{ color: '#aaffcc', fontFamily: 'monospace', fontSize: 9, lineHeight: 14 }}>
                {`claimStatus (DB): ${freshClaimState.claimStatus}`}
              </Text>
              <Text style={{ color: '#aaffcc', fontFamily: 'monospace', fontSize: 9, lineHeight: 14 }}>
                {`userClaimReqStatus (DB): ${freshClaimState.userClaimRequestStatus}`}
              </Text>
              <Text style={{ color: '#aaffcc', fontFamily: 'monospace', fontSize: 9, lineHeight: 14 }}>
                {`claimQuerySucceeded: ${freshClaimState.claimQuerySucceeded}`}
              </Text>
              <Text style={{ color: '#aaffcc', fontFamily: 'monospace', fontSize: 9, lineHeight: 14 }}>
                {`owner_id (DB): ${freshClaimState.ownerId || 'null'}`}
              </Text>
              <Text style={{ color: '#aaffcc', fontFamily: 'monospace', fontSize: 9, lineHeight: 14 }}>
                {`claimed_by (DB): ${freshClaimState.claimedBy || 'null'}`}
              </Text>
              <Text style={{ color: '#aaffcc', fontFamily: 'monospace', fontSize: 9, lineHeight: 14 }}>
                {`override: ${id && claimOverrides[id] ? JSON.stringify(claimOverrides[id]).slice(0, 80) : 'none'}`}
              </Text>
              <Text style={{ color: id ? (freshClaimState.ownerId === user.id ? '#00ff88' : '#ffaa44') : '#666', fontFamily: 'monospace', fontSize: 9, lineHeight: 14 }}>
                {`claimEnum: ${freshClaimState.claimEnum}`}
              </Text>
              <Text style={{ color: id ? (freshClaimState.ownerId === user.id ? '#00ff88' : '#ffaa44') : '#666', fontFamily: 'monospace', fontSize: 9, lineHeight: 14 }}>
                {`isOwner: ${isOwner} | isClaimed: ${isClaimed} | hasPending: ${hasPendingClaimRequest}`}
              </Text>
            </View>
          ) : null}
                ...
                {[...farmstandProducts]
                  ...
                  .map((product, index) => (
                    <ProductCard
                      ...
                      badges={product.seasonal ? ['seasonal'] : []}
                    />
                  ))}
              </ScrollView>
            </Animated.View>
