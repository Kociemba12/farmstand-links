...
export default function FarmerOnboardingScreen() {
  ...
  const [locationData, setLocationData] = useState<LocationInputData>(() => {
    const defaultData = createDefaultLocationData(isGuestUser ? 'cross_streets' : 'exact_address');
    // Pre-fill coordinates if passed via params
  ...
  const uploadFarmstandPhoto = useCallback(async (farmstandId: string): Promise<string | null> => {
    ...
    try {
      ...
      if (error) {
        ...
        // Don't throw - photo upload failure shouldn't block farmstand creation
        return null;
      }
      ...
      console.error('[Photo Upload] Unexpected error:', e);
      return null;
    } finally {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // IMPORTANT: Force geocoding to ensure coordinates match the entered address
    if (locationInputRef.current) {
      const geocodeSuccess = await locationInputRef.current.forceGeocode();
      if (!geocodeSuccess) {
        Alert.alert('Map Location Required', 'Please add the farmstand address and confirm the map location before submitting.');
        return;
      }
    }
    ...
    // Final guard: coordinates must be present
    if (locationData.latitude === null || locationData.longitude === null) {
      Alert.alert('Map Location Required', 'Please add the farmstand address and confirm the map location before submitting.');
      return;
    }
    ...
    try {
      ...
      if (photoUri) {
        ...
        if (preUploadedPhotoUrl) {
          console.log('[Onboarding] Photo pre-uploaded, URL will be included in INSERT:', preUploadedPhotoUrl);
        }
      }
      ...
      if (isLoggedIn && wantsToClaimOwnership && newFarmstandId && user?.id) {
        Alert.alert(
          'Farmstand Created',
          'Your farmstand has been submitted. Now complete the claim request by uploading photo evidence.',
          [
            {
              text: 'Claim Now',
              onPress: () => {
                // Navigate to the farmstand page with claim modal flag
                router.replace({
                  pathname: '/farm/[id]',
        ...
        Alert.alert(
          'Thanks! Your Farmstand was submitted.',
          "It's now visible on the map as Pending Approval. An admin will review it soon.",
          [
            {
              text: 'Done',
              onPress: () => {
                // Go back to map to see the new farmstand
                router.replace('/(tabs)/map');
              },
            },
          ]
  ...
  }, [farmName, hasProducts, acknowledgedDisclaimer, isLoggedIn, wantsToClaimOwnership, locationData, isGuestUser, user, selectedCategories, otherProducts, photoUri, createAdminFarmstand, uploadFarmstandPhoto, generateTempId, loadAlerts, router]);
