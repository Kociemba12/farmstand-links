...
import { Farmstand, OperationalStatus, SeasonalDates, HoursSchedule, HoursDay } from '@/lib/farmer-store';
...
import { uploadToSupabaseStorage, isSupabaseConfigured, ensureSessionReady } from '@/lib/supabase';
...
export default function OwnerEditScreen() {
      ...
      if (farmstandId) {
        ...
        if (fs) {
          ...
          const data: FormData = {
            ...
            operationalStatus: fs.operationalStatus || 'active',
    ...
    try {
      ...
      if (originalData) {
        const fieldsToTrack: (keyof FormData)[] = [
          ...
          'addressLine1', 'city', 'state', 'zip', 'operationalStatus', 'seasonalNotes',
      ...
      console.log('[EditListing] Refreshing farmstand data from Supabase...');
      await refreshSingleFarmstand(farmstandId);
  ...
  // Show "Claim pending approval" screen when user doesn't have owner_id match yet
  if (!canManage) {
    return (
      ...
      >
        ...
        >
          ...
          <Section
            ...
            title="Photos"
            subtitle="Add up to 8 photos. First photo is your main image."
            delay={100}
          >
            ...
            <View className="mt-4 pt-4 border-t border-gray-100">
              <View className="flex-row items-center justify-between mb-3">
                <View className="flex-1 mr-4">
                  <Text className="text-sm font-medium text-gray-700">Honor System / Self-Serve</Text>
                  <Text className="text-xs text-gray-500">Customers pay without staff present</Text>
                </View>
                <Switch
                  value={formData.honorSystem}
                  onValueChange={(v) => updateField('honorSystem', v)}
                  trackColor={{ false: '#d1d5db', true: '#86efac' }}
                  thumbColor={formData.honorSystem ? '#16a34a' : '#9ca3af'}
                />
              </View>
            </View>
          </Section>
          ...
          >
              ...
              >
                <View className="flex-row items-center">
                  <View
                    ...
                    style={{ backgroundColor: OPERATIONAL_STATUS_OPTIONS.find((s) => s.id === formData.operationalStatus)?.color }}
                  />
                  <Text className="text-base text-gray-900">
                    {OPERATIONAL_STATUS_OPTIONS.find((s) => s.id === formData.operationalStatus)?.label}
                  </Text>
                </View>
            ...
            {formData.operationalStatus === 'seasonal' && (
              <Pressable
          ...
          <Animated.View entering={FadeInDown.duration(200)} className="bg-white rounded-t-3xl pt-2 pb-8">
            ...
            {OPERATIONAL_STATUS_OPTIONS.map((option) => (
              <Pressable
                key={option.id}
                onPress={() => {
                  updateField('operationalStatus', option.id);
              ...
              >
                ...
                {formData.operationalStatus === option.id && (
