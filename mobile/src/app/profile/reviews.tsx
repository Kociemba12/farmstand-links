...
export default function ReviewsScreen() {
  ...
  // Reviews use userName to identify author
  const userReviews = useMemo(() => {
    if (!user?.id) return [];

    return allReviews
      .filter((review) => {
        // Must be authored by current user (match by userName or check if review has userId field)
        const reviewWithUserId = review as { userId?: string };
        const isAuthor =
          reviewWithUserId.userId === user.id ||
          review.userName === user.name ||
          review.userName === user.email;

        if (!isAuthor) return false;

        // Exclude demo reviews if flagged
        const reviewWithDemo = review as { isDemo?: boolean };
        if (reviewWithDemo.isDemo) return false;

        return true;
      })
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [allReviews, user?.id, user?.name, user?.email]);
