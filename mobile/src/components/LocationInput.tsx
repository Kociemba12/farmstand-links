  ...
  return (
    <View className="rounded-xl overflow-hidden border border-sand">
      ...
      <View style={styles.mapContainer}>
        ...
        {/* "Finding address…" overlay after pin drop */}
        {isReverseGeocoding && (
          <View
            style={{
              position: 'absolute',
        ...
        {!hasPin && !isLocating && !isReverseGeocoding && (
...
export const LocationInput = forwardRef<LocationInputRef, LocationInputProps>(function LocationInput({
  ...
  // Silently capture user's GPS to use as autocomplete proximity bias.
  // Priority: last-known position (fast) → current position (accurate) → null
  const userGpsRef = useRef<{ latitude: number; longitude: number } | null>(null);
  useEffect(() => {
    (async () => {
      try {
        const { status } = await Location.getForegroundPermissionsAsync();
        if (status !== 'granted') {
          console.log('[LocationInput] GPS permission not granted — proximity bias unavailable');
          return;
        }
        // Try last-known first (instant)
        const last = await Location.getLastKnownPositionAsync({ maxAge: 30 * 60 * 1000 });
        if (last) {
          userGpsRef.current = { latitude: last.coords.latitude, longitude: last.coords.longitude };
          console.log('[LocationInput] GPS cached (lastKnown):', userGpsRef.current.latitude.toFixed(4), userGpsRef.current.longitude.toFixed(4));
        }
        // Also fire a fresh getCurrentPosition in the background — updates the ref if it comes back
        // This is non-blocking: if it finishes before the user types, great; otherwise last-known is used
        Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.Balanced }).then((pos) => {
          userGpsRef.current = { latitude: pos.coords.latitude, longitude: pos.coords.longitude };
          console.log('[LocationInput] GPS refreshed (current):', userGpsRef.current.latitude.toFixed(4), userGpsRef.current.longitude.toFixed(4));
        }).catch(() => {
          // Non-critical — last-known position or no proximity bias will be used
        });
      } catch {
        // Silent — proximity is optional
      }
    })();
  }, []);
  ...
  const searchAddresses = useCallback(async (query: string) => {
    ...
    // Proximity priority (SOFT bias only — never restricts nationwide search):
    //   1. Form coordinates already set (e.g. after a prior autocomplete selection)
    //   2. Cached GPS location (set on mount from getLastKnownPositionAsync)
    //   3. undefined → Mapbox searches all of US with no geographic bias
    // Never fall back to a hardcoded region center (e.g. Oregon) — that skews results
    // toward one state even when the user is searching a completely different location.
    const hasFormCoords = value.latitude && value.longitude &&
      !isDefaultCoordinates(value.latitude, value.longitude);
    const proximity: { latitude: number; longitude: number } | undefined =
      hasFormCoords
        ? { latitude: value.latitude!, longitude: value.longitude! }
        : userGpsRef.current ?? undefined;

    const suggestions = await searchAddressSuggestions(query, proximity);
    setAddressSuggestions(suggestions);
    // Show the dropdown even on 0 results so the manual-entry fallback row can appear
  ...
  const triggerAutocompleteSearch = useCallback((query: string) => {
    ...
    // 300ms debounce — search on every keystroke, no minimum word requirement
      ...
      if (currentValue.locationMode === 'exact_address') {
        ...
        try {
          ...
          if (result) {
            ...
            // Geocode failed — guide user to drop a pin on the map
            setGeocodeError("Couldn't locate this address automatically. Please drop a pin on the map below to confirm the location.");
            setIsGeocoding(false);
            return false;
          }
        } catch (error) {
        ...
        setLocationError('Please use "My Location" to detect your coordinates, or drop a pin on the map.');
        return false;
      }
  ...
  const handleMapPress = useCallback(
    async (event: MapPressEvent) => {
      ...
      // Snapshot the current value before async work to avoid stale closure issues
      const baseUpdate = {
        ...value,
        latitude: lat,
        longitude: lng,
      ...
      try {
        ...
        if (reverseResults.length > 0) {
          ...
          const streetName = r.street || '';
          const streetAddress = streetNumber
            ? `${streetNumber} ${streetName}`.trim()
            : streetName;
          const zip = r.postalCode || '';
          const displayText = streetAddress
            ? `${streetAddress}, ${city}, ${state} ${zip}`.trim()
            : city && state
            ? `${city}, ${state}`
            : '';
          ...
          onChange({
            ...baseUpdate,
            addressLine1: streetAddress || baseUpdate.addressLine1,
            city: city || baseUpdate.city,
            state: state || baseUpdate.state,
            zip: zip || baseUpdate.zip,
            nearestCityState:
              city && state ? `${city}, ${state}` : baseUpdate.nearestCityState,
            displayLocationText: displayText || baseUpdate.displayLocationText,
          });
        }
      } catch {
        // Coordinates are already saved — address fields stay as-is
      } finally {
  ...
  const handleAddressBlur = useCallback(() => {
    ...
    // Hide suggestions after a delay — onPressIn fires before blur, so this is just cleanup
    setTimeout(() => {
      setShowSuggestions(false);
    }, 500);
  ...
  return (
    <View className="gap-4">
      ...
      {value.locationMode === 'exact_address' && (
        <Animated.View entering={FadeIn} className="gap-4">
          ...
          <View>
            ...
            {/* Autocomplete Dropdown — inline below input, no absolute positioning */}
            {showSuggestions && (
              <View style={styles.suggestionsContainer}>
                {addressSuggestions.map((item, index) => (
                  <React.Fragment key={item.id}>
                    ...
                    <Pressable
                      onPressIn={() => handleSuggestionSelect(item)}
                      style={({ pressed }) => [
                        styles.suggestionItem,
                        pressed && styles.suggestionItemPressed,
                        index === 0 && styles.suggestionItemFirst,
                        index === addressSuggestions.length - 1 && addressSuggestions.length > 0 && styles.suggestionItemLast,
                      ]}
                    >
                ...
                {/* Manual-entry fallback row — shown when search returns no results */}
                {addressSuggestions.length === 0 && !isLoadingSuggestions && value.addressLine1.trim().length >= 3 && (
                  <Pressable
                    onPressIn={() => {
                      // Use GPS if available, otherwise default to the Oregon fallback center
                      const fallbackLat = userGpsRef.current?.latitude ?? 45.22716;
                      const fallbackLng = userGpsRef.current?.longitude ?? -122.52339;
                      const typedAddress = value.addressLine1.trim();
                    ...
                    style={({ pressed }) => [
                      styles.suggestionItem,
                      styles.suggestionItemFirst,
                      styles.suggestionItemLast,
                      pressed && styles.suggestionItemPressed,
                    ]}
                  >
                    <View className="flex-row items-start">
                      <MapPin size={16} color="#8B6F4E" style={{ marginTop: 2, marginRight: 10 }} />
                      <View className="flex-1">
                        <Text className="text-charcoal font-medium text-sm" numberOfLines={1}>
                          No exact match found — use this address anyway
                        </Text>
                        <Text className="text-wood text-xs mt-0.5" numberOfLines={1}>
                          Continue entering city, state, and ZIP below
                        </Text>
                      </View>
                    </View>
                  </Pressable>
          ...
          <View className="bg-cream/80 rounded-xl p-4 border border-sand/40">
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center">
                {hasPin ? (
                  <CheckCircle2 size={18} color={value.geoSource === 'GEOCODED' ? '#2D5A3D' : '#D97706'} />
                ) : (
            ...
            {hasPin ? (
              <View className="mt-2">
                {value.geoSource === 'GEOCODED' ? (
                  <Text className="text-forest text-sm font-medium">Location verified</Text>
                ) : (
                  <Text className="text-amber-600 text-sm font-medium">Location set manually — confirm pin below</Text>
                )}
              ...
              <Text className="text-wood text-sm mt-2">
                Pin this farmstand on the map — tap below or use "Locate on Map"
              </Text>
            )}
          </View>
      ...
      {value.locationMode === 'use_my_location' && (
        <Animated.View entering={FadeIn} className="gap-4">
          {/* Show button only when no location captured yet; replace with confirmation once set */}
          {!hasPin ? (
            <Pressable
            ...
            <Animated.View
              entering={FadeIn}
              className="flex-row items-center gap-3 px-4 py-3.5 rounded-xl bg-green-50 border border-green-200"
            >
              <CheckCircle2 size={18} color="#16A34A" />
              <Text className="text-green-800 font-medium text-sm flex-1">
                Location set — drag pin to fine-tune
              </Text>
            </Animated.View>
          )}
