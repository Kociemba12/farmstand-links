...
import { ChevronDown, MapPin, Clock, Phone as PhoneIcon, CreditCard, Copy, Check, Mail } from 'lucide-react-native';
import Animated, { useAnimatedStyle, FadeIn, FadeOut, SharedValue } from 'react-native-reanimated';
import * as Clipboard from 'expo-clipboard';
import * as Haptics from 'expo-haptics';

export type FarmstandInfoIconType = 'verified' | 'location' | 'hours' | 'phone' | 'payments' | 'email';
...
const IconImage = ({ type, size = ICON_SIZE }: { type: FarmstandInfoIconType; size?: number }) => {
  const renderIcon = () => {
    switch (type) {
      ...
      case 'email':
        return <Mail size={size} color={ICON_COLOR} strokeWidth={1.5} />;
      case 'payments':
