...
#### Admin Access Diagnostics Panel (temporary, TestFlight debug)
- On the Profile screen, only for `contact@farmstand.online`, an expandable **"Admin Access Debug"** card appears below the Admin Dashboard button.
- Displays all 13 diagnostic values: email, user id, authSession, token expiry, currentSession, getValidSession result, admin role lookup, profile row, avatar_url, blocking reason, last error, supabase host, SecureStore status.
- **"Refresh Admin Check" button**: re-runs `getAdminAccessDiagnostics()` live.
- **"Refetch Profile Avatar" button**: re-runs `fetchProfileAvatarUrl()` and shows result inline.
- Tapping **Admin Dashboard** as the admin account now runs the full diagnostic first and shows an exact Alert with the blocking reason if AdminGuard would reject the session, before navigating.
- Implemented in: `src/lib/supabase.ts` → `getAdminAccessDiagnostics()` + `AdminAccessDiagnostics` interface; `src/app/(tabs)/profile.tsx` → `AdminDiagnosticsPanel` component.
...
└── lib/
    ...
    ├── products-store.ts    # Products catalog — Supabase-backed CRUD with AsyncStorage fallback
...
## Farmstand Manager Module

A premium financial management module for farmstand owners, accessible from My Farmstand.

### Features

- **Overview Tab**: Summary cards (Revenue, Expenses, Net Profit, Inventory Value), date range selector, recent activity feed, top-selling items, low inventory alerts
- **Inventory Tab**: Full CRUD for inventory items with stock status badges (In Stock, Low, Out), quick +/- quantity adjust, search, add/edit modal
- **Sales Tab**: Full CRUD for sales entries with optional inventory deduction, payment method tracking, period totals
- **Expenses Tab**: Full CRUD for expenses with category picker (feed, seed, packaging, supplies, labor, fees, utilities, other), vendor tracking
- **Reports Tab**: Printable report view with revenue/expense summary, top sellers, expense breakdown by category, recent transactions, and Share export

### Architecture

All Farmstand Manager code is fully isolated from existing claim/approval/listing/messaging flows.

**New Files:**
- `src/lib/manager-types.ts` — TypeScript types for InventoryItem, Sale, Expense, DateRange, ManagerSummary, ReportData
- `src/lib/manager-service.ts` — Supabase service functions for all CRUD operations and report building
- `src/components/FarmstandManager.tsx` — Main entry card + full-screen modal shell with tab navigation
- `src/components/manager/ManagerShimmer.tsx` — Loading skeleton components
- `src/components/manager/SummaryCards.tsx` — 2x2 summary card grid
- `src/components/manager/OverviewTab.tsx` — Dashboard overview with activity feed
- `src/components/manager/InventoryTab.tsx` — Inventory management CRUD
- `src/components/manager/SalesTab.tsx` — Sales tracking CRUD
- `src/components/manager/ExpensesTab.tsx` — Expense tracking CRUD
- `src/components/manager/ReportsTab.tsx` — Reports and export

**Modified Files:**
- `src/app/owner/my-farmstand.tsx` — Added FarmstandManager component import and render inside Manage tab (only for claimed farmstands)
- `src/lib/supabase.ts` — Added `gte()` and `lte()` methods to SupabaseQueryBuilder for date range filtering

**Supabase Migration:**
- `supabase-farmstand-manager.sql` — Creates `farmstand_inventory`, `farmstand_sales`, and `farmstand_expenses` tables with RLS policies scoped to `farmstand_owners` ownership table

### Safety Rules

- Module only appears for owners of claimed farmstands (`claimStatus === 'claimed'`)
- Zero modifications to existing claim_requests, farmstand listing, or admin approval tables or queries
- All new Supabase queries are scoped to the signed-in owner's farmstand(s) via RLS
- `gte()` and `lte()` additions to SupabaseQueryBuilder are purely additive

### Premium Downgrade / Re-upgrade Flow

When a claimed owner's `premiumStatus` is not `'trial'` or `'active'` (i.e. `'free'` or `'expired'`):
- **Farmstand Manager button still appears** — all claimed owners can open the manager
- **Tabs are still visible** in the tab bar with a small lock icon next to each label
- **Tab content is replaced** with a `PremiumLockedView` showing a lock icon, context-specific message, and a polished "Reactivate Premium" / "Upgrade to Premium" CTA button
- **No data is deleted** — sales, expenses, inventory, and report records remain intact in Supabase
- When Premium is re-activated, all saved data instantly reappears — no re-entry needed

The `FarmstandManager` component accepts a `premiumStatus` prop (defaults to `'free'`), which `my-farmstand.tsx` passes from the live farmstand record. An `onUpgradePress` callback navigates to the Premium screen.

## Owner Promotions (Phase 1)

A Phase 1 promotions flow for claimed farmstand owners to learn about and purchase paid visibility boosts.

### Entry Points (owner-only)
- **My Farmstand → Manage tab**: "Promotions" quick action row (Zap icon), subtitle "Boost visibility in Explore and on the map"
- **Premium Plan screen**: "About Promotions" info card is now tappable and navigates to the Promotions screen

### Screen: `/owner/promotions`

File: `src/app/owner/promotions.tsx`

**Header**: "Promote Your Farmstand"

**Explainer banner**: Green gradient explaining that promotions are optional paid boosts, separate from Premium.

**4 Promotion Option Cards**:
| Option | Duration | Type |
|---|---|---|
| Featured in Explore | 7 days | `featured_explore_7` |
| Featured in Explore | 30 days | `featured_explore_30` |
| Map Boost | 7 days | `map_boost_7` |
| Seasonal Spotlight | Seasonal | `seasonal_spotlight` |

Each card includes: name, description, duration, price placeholder ("Coming soon"), and a "Promote Now" CTA button.

**Active Promotions section**: Shows active/scheduled/expired promotions or an empty state message.

**How promotions work**: Info section listing 4 key facts about promotions.

### Payment Hooks (Phase 1)
- "Promote Now" buttons call a safe placeholder handler that shows an "Alert" explaining promotions are coming soon
- Each `PromotionOption` has a `paymentProductId: null` field — ready for RevenueCat one-time purchase product IDs
- `TODO` comments mark exactly where RevenueCat purchase calls should be wired
...
### Safety Rules
- Promotions entry points only appear in the owner Manage tab (claimed farmstand owners)
- No changes to existing premium, claims, analytics, or messaging logic
- Phase 1 does not implement any backend logic or ad ranking
